...
uses
  ...
  SCardErr, WinSCard, WinSmCrd,DelayLib;
...
function Hex2Bin(Value: string): string;
function Bin2HexExt(const Value: string): string;
...
begin
  ...
  begin
    i := 1;
    repeat
      Result := Result + Chr(StrToInt('$' + Copy(Hex, i, 2)));
      i := i + 2;
    until i > Length(Hex);
  end;
end;
...
begin
  ...
  if FSelectReader <> '' then
    FStates.szReader := StrNew(PChar(FSelectReader + #0))
  else
...
var
  NewReaderListStr: string;
  NewReaderListStrSize: Integer;
begin
  while not Terminated do
  begin
    try
      if (SCardListReaders(FContext, nil, nil,
          NewReaderListStrSize) = SCARD_S_SUCCESS) and
          (NewReaderListStrSize <> Length(FReaderListStr)) then
      begin
        SetLength(NewReaderListStr, NewReaderListStrSize);

        if SCardListReaders(FContext, nil,
            Pointer(NewReaderListStr), NewReaderListStrSize) = SCARD_S_SUCCESS then
          if FReaderListStr <> NewReaderListStr then
          begin
            FReaderListStr := NewReaderListStr;
            Synchronize(Sync_ReaderList);
          end;
      end;
      ...
      begin
        ...
        FRet := SCardGetStatusChange(FContext, 3000, FStates, 1);
...
procedure TPCSCConnectorWatchThread.ExecForStatic;
var
  NewReaderListStr: string;
  NewReaderListStrSize: Integer;
begin
...
begin
  ...
  begin
    ...
    RetVar := SCardTransmit(FCardHandle, PSendPci, Pointer(SBuf), SLen, PReadPci, Pointer(RBuf), @RLen);

    if RetVar = SCARD_S_SUCCESS then
      Result := Bin2HexExt(Copy(RBuf, 1, RLen))
    else
    begin
      Result := '';
      if Assigned(FOnError) then
